...
def stop(root):root.destroy()
#Class for inserting new agent
class agent_insert:
    def __init__(self):top=self.top=Tk()
...
top.mainloop()
def insert(self):self.agent_key = str(random.randint(10000, 99999))
...
cur.execute(statement, (self.agent_key, self.nameins.get(), self.addrins.get(), self.phoneins.get(),
self.passwordins.get()))
messagebox.showinfo("Success", a)
con.commit()
def stop(self):self.top.destroy()
def admin_page(self):self.top.destroy()
Admin_Page()
class agent_login:
    def start(self,agent_key):top=self.top=Tk()
...
Label(self.frame, text="Enter Customer ID", font=('arial 12'),
fg='Black',bg="lightgreen").place(x=950, y=300)
...
if len(a)!=0:
    for i in a:(custid, cname, cmobile, cadd, vid,vdesc,vnum,vtype) = i
...
Label(self.frame,text=str(self.commission), font=('arial 10'),
fg='Black',bg="lightgreen").place(x=625,y=100)
'''
cur = con.cursor()
...
'''
top.mainloop()
def stop(self):self.treeview.delete(*self.treeview.get_children())
self.top.destroy()
def edit_customer(self):self.top.destroy()
a=edit_customer()
a.start(self.agent_key)
def add_customer(self):self.stop()
a=new_customer()
a.start(self.agent_key)
def delete_customer(self):a = 'Record deleted successfully for customer id =' + str(self.custid.get())
cur = con.cursor()
statement = "delete from customer where custid= '" + self.custid.get() + "' "
cur.execute(statement)
con.commit()
messagebox.showinfo("Success", a)
def logout(self):self.stop()
login()
#NEW-CUSTOMER
class new_customer:
    def start(self,agent_key):top = self.top = Tk()
...
self.type=StringVar()
ttk.Label(self.frame, text='CUSTOMER DETAILS').place(x=75, y=50)
Label(self.frame, text='Name').place(x=50, y=150)
Label(self.frame, text='Mobile_no').place(x=50, y=200)
Label(self.frame, text='Address').place(x=50, y=250)
...
Label(self.frame, text='VEHICLE DETAILS').place(x=675, y=50)
Label(self.frame, text='Description').place(x=650, y=150)
Label(self.frame, text='Vehicle_no').place(x=650, y=200)
Label(self.frame, text='Type').place(x=650, y=250)
...
def back(self):self.top.destroy()
a=agent_login()
a.start(self.agent_key)
def insert(self):self.cust_id = str(random.randint(10000, 99999))
...
cur.execute(statement1, (self.cust_id, self.name.get(), self.mobile.get(), self.address.get(),
self.agent_key))
...
self.type = StringVar()
style=ttk.Style()
style.configure("BW.TLabel",foreground="Black",background="#41acf4")
ttk.Label(self.frame, text="EDIT AGENT DETAILS", font=('arial 15'),style="BW.TLabel").place(x=75,
y=50)
Label(self.frame, text="Enter Customer ID", font=('arial 10'), fg='Black', bg='#41acf4').place(x=450,
y=150)
...
Label(self.frame, text="EDIT VEHICLE DETAILS", font=('arial 15'), fg='Black',bg='#41acf4').place(x=875, y=50)
...
Label(self.left,text="ADMINISTRATOR LOGIN",font=('arial 30bold'),fg='Black',bg='#4298f4').place(x=0,y=0)
...
Label(self.right, text="EDIT AGENT DETAILS", font=('arial 15'), fg='Black',
bg='#41acf4').place(x=800,y=20)
Label(self.right, text="Enter Agent ID", font=('arial 10'), fg='Black', bg='#41acf4').place(x=1000,
y=100)
...
for i in a:(key, name, add, no, pwd) = i
...
top.mainloop()
def new_agent(self):self.stop()
agent_insert()
def logout(self):self.stop()
login()
def delete(self):a = 'Record deleted successfully with agent id =' + str(self.agent_key.get())
cur = con.cursor()
...
Button(root, text='TAKE ME TO LOGIN PAGE', background="lightblue", font=('arial14'),command=self.login).place(x=480, y=450)
root.mainloop()
def login(self):
    self.root.destroy()
login()
